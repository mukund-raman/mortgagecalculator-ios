...
/// Manages the starred screen of the application and the scroll view that displays the different calculations made
class StarredViewController : UINavigationController {
    
    // MARK: - DEFINITION
    
    // The scroll view used in the navigation controller
    let scrollViewController = StarredScrollViewController()
    lazy var scrollView: UIScrollView = scrollViewController.scrollView
    ...
    static var starredViewController: StarredViewController? = nil
    
    /// The variable used to check if the group being created is starred or not
    var isGroupStarred: Bool = true
    ...
    var starredBackButton: UIButton = UIButton()
    ...
    override func loadView() {
        ...
        nav.tintColor = UIColor.lightOrange1
        nav.barTintColor = UIColor.darkBlue1
        StarredViewController.starredViewController = self
        
        // Sets up the custom back button
        self.starredBackButton.frame.origin = CGPoint(x: 0, y: 46)
        self.starredBackButton.frame.size = CGSize(width: 94, height: 44)
        ...
        backButtonLabel.frame.origin = CGPoint(x: 29.3333, y: self.starredBackButton.frame.height-backButtonLabel.frame.height)
        backButtonLabel.text = "Starred"
        backButtonLabel.textColor = .lightOrange2
        self.starredBackButton.addSubview(backButtonLabel)
        ...
        backButtonImageView.frame.origin = CGPoint(x: 8, y: self.starredBackButton.frame.height-backButtonImageView.frame.height)
        self.starredBackButton.addSubview(backButtonImageView)
        ...
        self.pushViewController(self.scrollViewController, animated: true)
    }
    ...
    /// - Parameter startingGroup: The group that the new calculation will belong in, if any
    @objc func openCalc() {
    ...
    @objc func createNewGroup() {
        ...
        self.isGroupStarred = true
        self.present(NewGroup.createGroupAlertController(isRecentsViewCtr: false), animated: true)
    }
    ...
    func renameGroup(forIndex index: Int) {
        ...
        self.popViewController(animated: true)
        self.present(NewGroup.renameGroupController(index), animated: true)
    }
}
